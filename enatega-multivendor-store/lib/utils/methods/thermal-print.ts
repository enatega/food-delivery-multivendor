...
import { PermissionsAndroid, Platform, Alert, ToastAndroid, Linking } from "react-native";
...
export const formatReceipt = (order) => {
  ...
  const itemsText = order.items
    ?.map((item) => {
      ...
      const variationText = item.variation?.title ? ` (${item.variation.title})` : "";
