...
const ReviewCard = ({ theme, name, rating, description, date }) => {
  return (<View style={styles.cardContainer(theme)}>
    <TextDefault bold H5 textColor={theme.gray700}>{name}</TextDefault>
    <View style={styles.reviewContainer}>
      <StarRating numberOfStars={5} rating={rating}/>
      <TextDefault textColor={theme.gray500}>{date} Days ago</TextDefault>
    </View>
    <TextDefault textColor={theme.gray500} numberOfLines={5}>{description}</TextDefault>
  </View>)
}
