...
import { MaterialCommunityIcons, FontAwesome5, Ionicons, SimpleLineIcons } from '@expo/vector-icons'
...
function RestaurantDetailHeader({ restaurant, configuration, currentTheme, t, navigation, onOpenSearch }) {
  const [toggle, setToggle] = useState(false)

  const currentDayShort = new Date().toLocaleString('en-US', { weekday: 'short' }).toUpperCase()

  const todayOpeningTimes = restaurant?.openingTimes?.find((opening) => opening.day === currentDayShort)

  const handleNavigateToReviews = useCallback(() => {
    navigation.navigate('Reviews', {
      restaurantObject: { ...restaurant },
      PRE_NAVIGATION_TAB: 'STORE_TAB'
    })
  ...
  return (
    <View style={styles(currentTheme).mainContainer}>
      <View style={styles(currentTheme).imageContainer}>
        <ShimmerImage imageUrl={restaurant?.image} style={styles(currentTheme).mainRestaurantImg} />
        ...
        <View style={styles(currentTheme).headerIconsContainer}>
          <TouchableOpacity activeOpacity={0.7} style={styles(currentTheme).iconButton} onPress={() => navigation.goBack()}>
            <Ionicons name='arrow-back' color={currentTheme.white} size={scale(17)} />
          </TouchableOpacity>

          <View style={{ flexDirection: 'row', gap: scale(8) }}>
            <TouchableOpacity activeOpacity={0.7} style={styles(currentTheme).iconButton} onPress={onOpenSearch}>
              <Ionicons name='search' size={scale(17)} color={currentTheme.white} />
            </TouchableOpacity>

            <TouchableOpacity activeOpacity={0.7} style={styles(currentTheme).iconButton} onPress={handleNavigateToAbout}>
              <SimpleLineIcons name='info' size={scale(17)} color={currentTheme.white} />
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View style={styles(currentTheme).deliveryDetailsOverlay}>
          ...
          <View style={styles(currentTheme).detailPill}>
            <TextDefault textColor={currentTheme.fontMainColor}>
              {t('minimumOrder')} {configuration.currencySymbol} {restaurant?.minimumOrder}
            </TextDefault>
          </View>
        </View>
      </View>
      ...
      <View style={styles(currentTheme).contentContainer}>
        ...
        <View style={styles(currentTheme).subContainer}>
          <View style={styles(currentTheme).titleContainer}>
            <Image resizeMode='cover' source={restaurant?.logo ? { uri: restaurant.logo } : require('../../../assets/images/defaultLogo.png')} style={styles(currentTheme).restaurantImg} />
            <TextDefault numberOfLines={1} H3 bolder textColor={currentTheme.fontThirdColor}>
        ...
        <View style={styles(currentTheme).cuisineContainer}>
          <TextDefault textColor={currentTheme.fontThirdColor} H5 bold>
            {toggle ? restaurant?.cuisines?.join(', ') : restaurant?.cuisines?.join(', ').substring(0, 40) + '...'}
          </TextDefault>
          {restaurant?.cuisines?.toString()?.length > 40 && <IconButton icon={toggle ? 'arrow-up' : 'arrow-down'} iconColor='gray' style={{ width: 25 }} onPress={() => setToggle((prev) => !prev)} />}
        </View>
        ...
        <View style={styles(currentTheme).infoContainer}>
          ...
          <TouchableOpacity style={styles(currentTheme).reviewButton} activeOpacity={0.8} onPress={handleNavigateToReviews}>
        ...
        <View style={styles(currentTheme).timingContainer}>
          <View style={styles(currentTheme).ratingBox}>
            <MaterialCommunityIcons name='timer-outline' size={scale(21)} color={currentTheme.newIconColor} />
            {todayOpeningTimes && (
              <View style={styles(currentTheme).timingRow}>
                ...
                {todayOpeningTimes?.times?.length < 1 ? (
                  ...
                  todayOpeningTimes?.times?.map((timing, index) => (
                    <TextDefault key={index} textColor={currentTheme.fontThirdColor} bold>
          ...
          <TouchableOpacity style={styles(currentTheme).statusButton} disabled={true}>
            <TextDefault bolder textColor={currentTheme.main}>
