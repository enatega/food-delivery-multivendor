...
import { GIVE_FEEDBACK, REVIEW_ORDER_SINGLE_VENDOR } from '../../apollo/mutations'

const REVIEWORDER = gql`
  ${REVIEW_ORDER_SINGLE_VENDOR}
`
const GIVEFEEDBACK = gql`
  ${GIVE_FEEDBACK}
`
...
const FeedBack = () => {
  ...
  const fromProfileHelpFeedback = route?.params?.fromProfileHelpFeedback === true
  ...
  const [submitReview, { loading: submittingReview }] = useMutation(REVIEWORDER, {
    onCompleted: (data) => {
  ...
  const [submitFeedback, { loading: submittingFeedback }] = useMutation(GIVEFEEDBACK, {
    onCompleted: () => {
      FlashMessage({ message: t('Feedback submitted successfully') || 'Feedback submitted successfully' })
      navigation.goBack()
    },
    onError: (error) => {
      console.log('Give feedback error:', JSON.stringify(error))
      FlashMessage({
        message: error?.networkError?.result?.errors?.[0]?.message || error?.message || t('Something went wrong, please try again!') || 'Something went wrong, please try again!'
      })
    }
  ...
  const isSubmitting = submittingReview || submittingFeedback
  const isSubmitEnabled = rating > 0 && feedbackText.trim().length > 0 && !isSubmitting
