...
const UserOnboarding = () => {
  ...
  useFocusEffect(() => {
    if (Platform.OS === 'android') {
      StatusBar.setBackgroundColor(currentTheme?.colorBgSecondary)
    }
  ...
  return (
    <SafeAreaView style={styles(currentTheme).container}>
        <ScrollView 
        contentContainerStyle={styles(currentTheme, insets).contentContainer}
        showsVerticalScrollIndicator={false}
        style={styles(currentTheme, insets).onBoardingContainer }>
          <MainHeading />
          <View style={styles(currentTheme, insets).imageSection}>
            <MainImage />
          </View>
          <Actions t={t} />
        </ScrollView>
