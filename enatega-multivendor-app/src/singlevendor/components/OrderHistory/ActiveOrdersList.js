...
const ActiveOrdersList = ({ onOrderPress, currentTheme: passedTheme }) => {
  ...
  // Transform and update active orders
  useEffect(() => {
    if (orders && orders.length > 0) {
      const currencySymbol = configuration?.currencySymbol || ''
      const transformedItems = buildActiveOrderList({
        orders: orders,
        currencySymbol,
        t
      })
      ...
      if (page === 1) {
        ...
        setActiveOrders(transformedItems)
      } else {
  ...
  const handleLoadMore = useCallback(async () => {
    ...
    try {
      ...
      if (data?.getUsersActiveOrders?.length === 0) {
        setIsLoadingMore(false)
      }
    } catch (error) {
      console.error('Error loading more active orders:', error)
      setIsLoadingMore(false)
  ...
  const flatListData = useMemo(() => {
    ...
    if (activeOrders.length > 0) {
      ...
      } else if (pageInfo?.isEndReached && activeOrders.length > 0) {
        items.push({
          type: 'endMessage',
          id: 'endMessage-active'
        })
      }
    }
    
    return items
  }, [activeOrders, isLoadingMore, pageInfo, handleLoadMore])
