...
const TipBottomSheet = React.forwardRef(({ onTipSelected, currencySymbol = '€', currentTip = 0 }, ref) => {
  ...
  useEffect(() => {
    if (currentTip > 0) {
      ...
      if (!predefinedTips.includes(currentTip)) {
        setSelectedPredefinedTip('custom');
      } else {
        setSelectedPredefinedTip(currentTip);
      }
    }
  }, [currentTip]);
  ...
  const clampTipValue = (value) => Math.min(Math.max(value, MIN_TIP), MAX_TIP);

  const handleInputChange = (text) => {
    // Allow only one decimal point
    const sanitized = text.replace(/[^0-9.]/g, '');
    const parts = sanitized.split('.');
    const cleanedText = parts.length > 2 ? `${parts[0]}.${parts.slice(1).join('')}` : sanitized;
  ...
  const handleDone = () => {
    ...
    if (!isNaN(amount)) {
      ...
      const clamped = clampTipValue(amount);
      if (clamped >= MIN_TIP && clamped <= MAX_TIP) {
        onTipSelected(clamped);
        ref?.current?.close();
      }
    }
  };
  ...
  const handleInputBlur = () => {
    ...
    if (!isNaN(amount)) {
      const clamped = clampTipValue(amount);
      setTipAmount(clamped.toFixed(2));
    }
  };
  ...
  const isValidAmount = () => {
    ...
    return amount >= MIN_TIP && amount <= MAX_TIP;
  };
