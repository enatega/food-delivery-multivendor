  ...
  return (
    <TouchableOpacity style={styles(currentTheme).itemRow} onPress={() => setShowDetails(!showDetails)} activeOpacity={0.7}>
      ...
      <View style={styles().itemContent}>
        ...
        <TextDefault textColor={currentTheme.gray500} small bold isRTL 
      ...
      <View style={styles().itemRight}>
        <TextDefault textColor={currentTheme.fontMainColor} isRTL>
          {currencySymbol} {(item?.variation?.price * item?.foodQuantity).toFixed(2)}
        </TextDefault>
        {/* <Feather name={showDetails ? 'chevron-up' : 'chevron-down'} size={16} color={currentTheme.gray400} /> */}
      </View>
    </TouchableOpacity>
  )
}

const OrderItemsSection = ({ items = [], currencySymbol = '€', initialExpanded = false }) => {
  ...
  const getItemsSummary = () => {
    const names = items.map((item) => item.foodTitle).join(' + ')
    return names.length > 30 ? names.substring(0, 30) + '...' : names
  }

  return (
    <View style={styles(currentTheme).container}>
      <TextDefault textColor={currentTheme.fontMainColor} H5 bolder isRTL style={styles().sectionTitle}>
      ...
      {items.length == 1 ? null : (
        <TouchableOpacity style={styles().collapsedRow} onPress={() => setExpanded(!expanded)} activeOpacity={0.7}>
          <View style={styles().imageStack}>
            ...
            {remainingCount > 0 && (
              <View style={[styles(currentTheme).remainingBadge, { marginLeft: -scale(12) }]}>
                <TextDefault textColor={currentTheme.fontMainColor} small bold>
                  +{remainingCount}
                </TextDefault>
              </View>
            )}
          ...
          <View style={styles().summaryContent}>
            <TextDefault textColor={currentTheme.fontMainColor} isRTL numberOfLines={1} style={styles().summaryText} bold>
              {getItemsSummary()}
            </TextDefault>
          </View>
          <Feather name={expanded ? 'chevron-up' : 'chevron-down'} size={20} color={currentTheme.fontMainColor} />
        </TouchableOpacity>
      )}

      {/* Expanded View - Item List */}
      {(expanded || items.length == 1) && (
        <View style={[styles().itemsList, { marginTop: items.length == 1 ? 3 : undefined }]}>
          {items.map((item, index) => (
            <OrderItemRow key={item.id || index} item={item} currencySymbol={currencySymbol} currentTheme={currentTheme} isExpanded={true} />
          ))}
        </View>
      )}
    </View>
