...
import { Divider, Grid, LinearProgress, Typography, useMediaQuery, useTheme } from "@mui/material";
...
function MyOrders() {
  ...
  const { orders, loadingOrders, errorOrders, fetchOrders } = useContext(UserContext);
  const activeOrders = orders.filter((o) => ACTIVE_STATUS.includes(o.orderStatus));
  const pastOrders = orders.filter((o) => INACTIVE_STATUS.includes(o.orderStatus));
useEffect(async()=>{
  ...
  if (loadingOrders) {
    return (
      <Grid container className={classes.root}>
        <Header />
        <div style={{ width: "100%" }} className={clsx(classes.loadingContainer, classes.center)}>
