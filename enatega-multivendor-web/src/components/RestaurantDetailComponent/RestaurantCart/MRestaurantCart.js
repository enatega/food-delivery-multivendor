...
import { Button, CircularProgress, Container, Hidden, Typography, useTheme } from "@mui/material";
...
function MRestaurantCart(props) {
  ...
  const { cart, cartCount, clearCart, updateCart, restaurant: cartRestaurant } = useContext(UserContext);
    ...
    try {
      if (cart && cartCount) {
        const transformCart = cart.map((cartItem) => {
          ...
          const variationItem = foodItem.variations.find((variation) => variation._id === cartItem.variation._id);
  ...
  return (
    <Hidden lgUp>
      <RouterLink to={loadingTax ? "#" : "/checkout"} style={{ textDecoration: "none" }}>
        <Button
          disabled={loadingTax}
          style={{
        ...
        >
          {loadingTax || loadingData || loading ? (
            ...
            <>
              ...
              >
                <Typography variant="caption" align="left" className={clsx(classes.mobileCartText, classes.whiteText)}>
                  {cartCount}
                </Typography>
                <Typography
                  style={{
                ...
                <Typography variant="body2" align="right" className={clsx(classes.mobileCartText, classes.whiteText)}>
