...
export default function SingleVendorBrowse() {
  ...
  useEffect(() => {
    const timeout = window.setTimeout(() => {
      if (term.trim()) void search({ variables: { search: term.trim() } });
    }, 300);
  ...
  return <div className="pb-12 pt-5">
    ...
    {term ? <SingleVendorProductSection title={result.loading ? "Searching…" : "Search results"} products={normalizeProducts(result.data?.searchFood)} /> : <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">{(categories.data?.getRestaurantCategoriesSingleVendor ?? []).map((category: any) => <Link href={`/category/${category.id}`} key={category.id} className="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">{category.image && <Image src={category.image} alt="" width={320} height={320} className="aspect-square w-full rounded-xl object-cover" />}<p className="mt-3 font-semibold text-gray-900 dark:text-white">{category.name}</p><p className="text-sm text-gray-500">{category.itemCount ?? 0} products</p></Link>)}</div>}
  </div>;
}
