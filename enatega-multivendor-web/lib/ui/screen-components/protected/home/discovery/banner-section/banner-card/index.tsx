...
const BannerCard:React.FC<IBannerItemProps> = ({item}) => {
  ...
  const onClickHandler = () => {
    if (item?.action === 'Navigate Specific Restaurant') {
      router.push(`/${item?.shopType === 'restaurant' ? 'restaurant' : 'store'}/${item?.slug}/${item?.screen}`)
    } else {
    ...
    return (
      <>
        <div className="carousel-item relative cursor-pointer mx-[6px] md:mx-[12px]" onClick={onClickHandler}>
