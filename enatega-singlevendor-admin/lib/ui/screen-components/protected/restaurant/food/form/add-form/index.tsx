...
const FoodForm = ({ position = 'right' }: IFoodAddFormComponentProps) => {
  ...
  const getInitialValues = () => {
    ...
    const basicInfo: IFoodDetailsForm = {
      ...
      // ✅ NEW
      ingredients: foodContextData?.food?.data?.ingredients || '',
      usage: foodContextData?.food?.data?.usage || '',
      nutritionDetail: foodContextData?.food?.data?.nutritionDetail || '',
      nutritionFacts:
        (foodContextData?.food?.data?.nutritionFacts?.length ?? 0) > 0
          ? foodContextData?.food?.data?.nutritionFacts!
          : [{ name: '', quantity: '' }],
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      const cleanTypename = <T,>(obj: T): T => {
        if (Array.isArray(obj)) {
          return obj.map((item) => cleanTypename(item)) as unknown as T;
        }
        ...
        if (obj !== null && typeof obj === 'object') {
          ...
          const cleanedEntries = Object.entries(record)
            ...
            .map(([key, value]) => [key, cleanTypename(value)]);
      ...
      if (foodContextData?.isEditing) {
        ...
        const _variations: IVariationInput[] = values.variations.map((item) => {
          const variation: IVariationInput = {
            ...
            addons: item?.addons
              ? (item.addons
                  .map((addon: IDropdownSelectItem | string) =>
                    typeof addon === 'object' ? addon.code : addon
                  )
          ...
          if (item.deal) {
            ...
            if ('name' in item.deal && typeof item.deal.name === 'string') {
            ...
            } else if ('dealName' in item.deal && item.deal.dealName) {
              dealTitle = item.deal.dealName;
              dealDiscountType = item.deal.discountType || '';
              dealDiscountValue = item.deal.discountValue || 0;
              dealStartDate =
                item.deal.startDate instanceof Date
                  ? item.deal.startDate
                  : new Date(item.deal.startDate);
              dealEndDate =
                item.deal.endDate instanceof Date
                  ? item.deal.endDate
                  : new Date(item.deal.endDate);
        ...
        const foodInput: IFoodInput = {
          food: {
            ...
            usage: values.usage,
            nutritionDetail: values.nutritionDetail,
            nutritions: cleanedNutritionFacts, // :white_check_mark: Use cleaned version
            subCategory: (values.subCategory?.code ||
              values.subCategory ||
              undefined) as string,
        ...
        const _variations: IVariationInput[] = values.variations.map((item) => {
          ...
          // Add deal if present (same code as above)
          if (item.deal) {
            let dealTitle: string;
            let dealDiscountType: string;
            let dealDiscountValue: number;
      ...
      if (error instanceof Error) {
        ...
        if (
          ...
          message = (error.graphQLErrors as ApolloError['graphQLErrors'])[0]
            .message;
        }
      }
      showToast({
                      ...
                      <ul className="text-xs text-red-700 dark:text-red-300 list-disc list-inside">
                        {Object.entries(formErrors).map(([key, value]) => {
                          if (key === 'variations' && Array.isArray(value)) {
                            return (
                              value as Array<Record<string, string> | undefined>
                            ).map((varError, idx: number) => {
